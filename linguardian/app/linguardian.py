...
class Linguardian:
    ...
    def return_all_text(self, ocr_data):
        """Return all text with coordinates."""
        all_text = []
        for j, word in enumerate(ocr_data['text']):
    ...
    def process_pdf(self):
        ...
        try:
            ...
            page_text = self.return_all_text(ocr_data)
            all_ocr_data.append(page_text)
            ...
            return output_data, all_ocr_data
        except Exception as e:

            logger.error(f"Error during PDF processing: {e}")
            raise HTTPException(status_code=500, detail="Internal server error @ class level, while processing PDF.")  
